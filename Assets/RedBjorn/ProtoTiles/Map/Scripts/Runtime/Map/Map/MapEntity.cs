...
namespace RedBjorn.ProtoTiles
{
    ...
    public partial class MapEntity : IMapNode, IMapDirectional
    {
        ...
        public TileEntity BaseTile { get; private set; }
        public TileEntity CpuBaseTile { get; private set; }   
                                     ...
                                     List<TileEntity> result)
        {
            ...
            while (run)
            {
                ...
                if (tile == null || Distance(worldPosStart, WorldPosition(tile)) > maxDistance)
                {
                    break;
                }
                if (tile != current)
        ...
        /// <returns></returns>
        // public HashSet<TileEntity> VacantTiles(Vector3Int origin, float range)
        // {
        //     var nodes = NodePathFinder.VacantArea(this, Tile(origin), range, null);
        ...
        /// <returns></returns>
        public List<TileEntity> PathTiles(Vector3 from, Vector3 to, float range, List<TileOccupant> ignoreOccupantSet = null)
        {
            var nodes = NodePathFinder.Path(this, Tile(from), Tile(to), range, ignoreOccupantSet);
        ...
        /// <param name="range"></param>
        /// <returns></returns>
        public List<Vector3> PathPoints(Vector3 from, Vector3 to, float range, List<TileOccupant> ignoreOccupantSet = null)
        {
            var nodes = NodePathFinder.Path(this, Tile(from), Tile(to), range, ignoreOccupantSet);
        ...
        public List<Vector3> WalkableBorder(TileEntity origin, float range)
        {
            ...
            var walkable = NodePathFinder.VacantAreaPositions(this, origin, range);
        ...
        public void SpawnPresetEntities()
        {
            foreach (var (_, tile) in Tiles)
            {
                if (tile.SetToSpawnWood)
                {
                    new ResourceStructure(4, ResourceType.Wood, tile);
                }
                else if (tile.SetToSpawnMetal)
                {
                    new ResourceStructure(3, ResourceType.Metal, tile);
                }
                else if (tile.SetToSpawnBase)
                {
                    var tileSet = TileManager.instance.VacantSnowflake(tile);

                    new Base(tileSet, 2, Party.Player);
                    BaseTile = tile;
                }
                else if (tile.SetToSpawnHostileBase)
                {
                    var tileSet = TileManager.instance.VacantSnowflake(tile);

                    new Base(tileSet, 4, Party.CPU);
                    CpuBaseTile = tile;
                }
            }
        }
